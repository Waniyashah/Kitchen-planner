...
interface AreaSeparationModalProps {
  visible: boolean;
  onCancel: () => void;
  onOk: () => void;
  onUndo: () => void;
...
export default function AreaSeparationModal({
  visible,
  onCancel,
  onOk,
  onUndo,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      <div className="w-full max-w-6xl h-[80vh] rounded-lg bg-white shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex h-16 items-center justify-between border-b border-gray-200 px-6 flex-shrink-0">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Add an area separation</h2>
            <p className="text-sm text-gray-600">
              Select a point to start your separation wall. Select the end point to finish it. (Note- a separation wall is an invisible line to mark out an area, the eating area for example.)
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Button size="sm" onClick={onOk} className="bg-[#0051BA] hover:bg-[#003d8f]">
              Ok
            </Button>
          </div>
        </div>
